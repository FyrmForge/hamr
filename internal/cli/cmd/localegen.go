...
func init() {
	...
	genCmd.AddCommand(localeGenCmd)
}
...
func generateGoSource(pkg string, keys []localeKeyInfo) string {
	...
	b.WriteString("// Code generated by hamr gen locale; DO NOT EDIT.\n\n")
	fmt.Fprintf(&b, "//go:generate hamr gen locale\n\n")
...
func checkMethodNames(keys []localeKeyInfo) error {
	...
	for _, k := range keys {
		...
		if m == "" {
			return fmt.Errorf("gen locale: key %q produces an empty method name (no letters/digits)", k.key)
		}
		if prev, ok := seen[m]; ok {
			return fmt.Errorf("gen locale: keys %q and %q both generate method %s — rename one", prev, k.key, m)
		}
		seen[m] = k.key
	}
	return nil
