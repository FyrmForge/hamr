...
func TestGenerateProject_locale(t *testing.T) {
	...
	// "hamr gen locale" which runs as part of make build / make test.
	...
	// build and test targets should run hamr gen locale.
	assert.Contains(t, makefile, "hamr gen locale")

	agents := readFile(t, dir, "AGENTS.md")
	assert.Contains(t, agents, "internal/locale/         Generated type-safe i18n accessors after `hamr gen locale`")
...
func TestGenerateProject_noLocale(t *testing.T) {
	...
	assert.NotContains(t, makefile, "hamr gen locale")
}
