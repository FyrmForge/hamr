...
func TestSSEBroker_ConfigEvent(t *testing.T) {
	...
	broker := NewSSEBroker(rules, daemons, nil, false, false, false)
	srv := httptest.NewServer(broker.Handler())
	defer srv.Close()
...
func TestSSEBroker_ConfigEvent_MailMockFlag(t *testing.T) {
	t.Run("enabled", func(t *testing.T) {
		broker := NewSSEBroker(nil, nil, nil, true, false, false)
		srv := httptest.NewServer(broker.Handler())
		defer srv.Close()
...
func TestSSEBroker_ConfigEvent_StripeMockFlag(t *testing.T) {
	t.Run("enabled", func(t *testing.T) {
		broker := NewSSEBroker(nil, nil, nil, false, true, false)
		srv := httptest.NewServer(broker.Handler())
		defer srv.Close()
