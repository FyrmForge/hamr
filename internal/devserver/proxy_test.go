...
func TestInjectReloadScript_HTML(t *testing.T) {
	...
	handler := NewProxyHandler(backend.Listener.Addr().String(), broker, nil, nil, nil, true)
...
func TestInjectReloadScript_Disabled(t *testing.T) {
	...
	broker := NewSSEBroker(nil, nil, nil)
	handler := NewProxyHandler(backend.Listener.Addr().String(), broker, nil, nil, nil, false)
	proxy := httptest.NewServer(handler)
	defer proxy.Close()
...
func TestSSEEndpoint(t *testing.T) {
	broker := NewSSEBroker(nil, nil, nil)
	handler := NewProxyHandler("localhost:9999", broker, nil, nil, nil, true)
	proxy := httptest.NewServer(handler)
	defer proxy.Close()
...
func TestListenAndServeProxy(t *testing.T) {
	broker := NewSSEBroker(nil, nil, nil)
	handler := NewProxyHandler("localhost:9999", broker, nil, nil, nil, false)
...
func TestErrorPage_ServedOnBuildError(t *testing.T) {
	...
	broker := NewSSEBroker(nil, nil, nil)
	handler := NewProxyHandler(backend.Listener.Addr().String(), broker, es, nil, nil, true)
	proxy := httptest.NewServer(handler)
	defer proxy.Close()
...
func TestErrorPage_SkippedForAPI(t *testing.T) {
	...
	es.Set("go", "error")
	broker := NewSSEBroker(nil, nil, nil)
	handler := NewProxyHandler(backend.Listener.Addr().String(), broker, es, nil, nil, false)
	proxy := httptest.NewServer(handler)
	defer proxy.Close()
...
func TestLogsEndpoint(t *testing.T) {
	...
	broker := NewSSEBroker(nil, nil, nil)
	handler := NewProxyHandler("localhost:9999", broker, nil, logBuf, nil, false)
	proxy := httptest.NewServer(handler)
	defer proxy.Close()
